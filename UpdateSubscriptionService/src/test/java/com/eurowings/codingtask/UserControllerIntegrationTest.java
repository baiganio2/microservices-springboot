...
class UserControllerIntegrationTest {
	...
	public void test_create_user() {
		...
		given().port(port).body(userDto).contentType("application/json").post("/eurowings-user/api/v1/users").then()
	...
	public void test_update_subscription_status() {
		...
		ExtractableResponse<Response> updateResponseEx = given().port(port).body(subscriptionDto)
				.contentType("application/json").post("/eurowings-user/api/v1/users/subscriptionStatus").then().and()
				.extract();
