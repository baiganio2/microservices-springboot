...
class SubscriptionControllerIntegrationTest {
	...
	public void test_unauthorized_request() {
		...
		given().port(port).body(subscriptionDto).contentType("application/json")
				.post("/eurowings-subscriptions/api/v1/subscriptions").then().statusCode(HttpStatus.SC_UNAUTHORIZED);
	}
	...
	public void test_update_subscription_status() {
		...
		given().auth().basic("eurowings", "eurowings").port(port).body(subscriptionDto).contentType("application/json")
				.post("/eurowings-subscriptions/api/v1/subscriptions").then().statusCode(HttpStatus.SC_OK);
	}
	...
	public void test_get_subscription_status() {
		...
		given().auth().basic("eurowings", "eurowings").port(port).
		when().get("/eurowings-subscriptions/api/v1/subscriptions/"+2).
